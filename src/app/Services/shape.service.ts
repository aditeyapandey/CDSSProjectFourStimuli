...
export class ShapeService {
  ...
  drawRectangles(shape:string,widthRect:number,heightRect:number,featureArray:any)
  {
    ...
    let rectCoordinates=this.generateRectangleCoordinates(4,width,height,10);
    ...
    let fourthRect=(4*rectCoordinates.padding)+(3*rectCoordinates.rect)
    let sizeofpixel=7
    ...
    d3.select("#"+shape).append("circle").attr("cx",fourthRect+(rectCoordinates.rect/2)).attr("cy",3.5*(height/10)+(rectCoordinates.rect/2)).attr("r",(rectCoordinates.rect/2)).attr("stroke","black").style("fill","white").style("opacity",this.rectFill(featureArray[3]));
    ...
    if(shape=="instructionSVG2") {
      ...
      d3.select("#" + shape).append("line").attr("x1", fourthRect+(rectCoordinates.rect/2)).attr("x2", fourthRect+(rectCoordinates.rect/2)).attr("y1", 3.5*(height/10) ).attr("y2", 3.5*(height/10)-(1*rectCoordinates.rect)).attr("stroke", "red").style("stroke-width", 1)
      ...
      d3.select("#" + shape).append("text").attr("x", fourthRect).attr("y", 3.5*(height/10)-(1*rectCoordinates.rect)-sizeofpixel/2).text("Feature 4").attr("fill", "red").style("font-size", sizeofpixel+"px")
  ...
  drawFace(shape:string,widthFace:number,heightFace:number,featureArray:any)
  {
    ...
    if(featureArray[0]){
      //Eyebrows
      let arcPath = "M" + (cx - cx / 2.5) + "," + (cy - cy / 1.8) + " a1, 0.2, 0, 0,1 " + (r / 2) + ",0";
      let arcPath1 = "M" + (cx - cx / 2.5) + "," + (cy - cy / 1.8) + " a1, 0.2, 0, 0,0 " + (r / 2) + ",0";
      let arcPath3 = "M" + (cx- cx / 2.5 + r/2) + "," + (cy - cy / 1.8) + " a1, 0.2, 0, 0,1 " + (r / 2) + ",0";
      let arcPath4 = "M" + (cx- cx / 2.5 + r/2) + "," + (cy - cy / 1.8) + " a1, 0.2, 0, 0,0 " + (r / 2) + ",0";

      d3.select("#" + shape).append("path").attr("d",arcPath).attr("stroke","black").style("fill","white")
      d3.select("#" + shape).append("path").attr("d",arcPath1).attr("stroke","black").style("fill","white")
      d3.select("#" + shape).append("path").attr("d",arcPath3).attr("stroke","black").style("fill","white")
      d3.select("#" + shape).append("path").attr("d",arcPath4).attr("stroke","black").style("fill","white")

    }

    if(featureArray[1]) {
      //eyes
      d3.select("#" + shape).append("circle").attr("cx", cx - cx / 5).attr("cy", cy - cy / 4).attr("r", cx / 5).attr("stroke", "black").style("fill", "white");
      d3.select("#" + shape).append("circle").attr("cx", cx + cx / 5).attr("cy", cy - cy / 4).attr("r", cx / 5).attr("stroke", "black").style("fill", "white");
    }
    if(featureArray[2]) {
      //nose
      d3.select("#" + shape).append("line").attr("x1", cx).attr("y1", cy   ).attr("x2", cx).attr("y2", cy + cy / 2.5).attr("stroke", "black");
    }
    if(featureArray[3]) {
      //mouth
      d3.select("#" + shape).append("line").attr("x1", cx - cx / 5).attr("y1", cy + cy / 2).attr("x2", cx + cx / 5).attr("y2", cy + cy / 2).attr("stroke", "black");

    }
    if(shape=="instructionSVG2") {
      d3.select("#" + shape).append("line").attr("x1", cx - cx / 5).attr("x2", cx - cx / 2).attr("y1",cy - cy / 1.9).attr("y2",cy - cy+5 ).attr("stroke", "red").style("stroke-width", 1)
      d3.select("#" + shape).append("line").attr("x1", 0 ).attr("y1", cy - cy / 4).attr("x2", cx - cx / 5 ).attr("y2", cy - cy / 4 ).attr("stroke", "red").style("stroke-width", 1)
      d3.select("#" + shape).append("line").attr("x1", 0 ).attr("y1", cy + cy / 6).attr("x2", cx ).attr("y2", cy + cy / 6).attr("stroke", "red").style("stroke-width", 1)
      d3.select("#" + shape).append("line").attr("x1", cx).attr("y1", cy + cy / 2).attr("x2", cx-(cx/2)).attr("y2", cy+cy ).attr("stroke", "red").style("stroke-width", 1);
      ...
      d3.select("#" + shape).append("text").attr("x",cx - cx / 1.2 ).attr("y",cy - cy+4).text("Feature 1").attr("fill", "red").style("font-size", sizeOfPixel+"px")
      d3.select("#" + shape).append("text").attr("x",(-cx/1.5) ).attr("y",  cy - cy / 4.5).text("Feature 2").attr("fill", "red").style("font-size", sizeOfPixel+"px")
      d3.select("#" + shape).append("text").attr("x",(-cx/1.5) ).attr("y", cy + cy / 5).text("Feature 3").attr("fill", "red").style("font-size", sizeOfPixel+"px")
      d3.select("#" + shape).append("text").attr("x",cx-(cx/1.2)).attr("y",cy+cy+5).text("Feature 4").attr("fill", "red").style("font-size", sizeOfPixel+"px")
    }
    ...
    drawFaceScrambled(shape:string,widthFace:number,heightFace:number,featureArray:any)
    {
      ...
      if(featureArray[1]){
        //Eyebrows
        let arcPath = "M" + (cx - cx / 2.5) + "," + (cy - cy / 4.5) + " a1, 0.2, 0, 0,1 " + (r / 2) + ",0";
        let arcPath1 = "M" + (cx - cx / 2.5) + "," + (cy - cy / 4.5) + " a1, 0.2, 0, 0,0 " + (r / 2) + ",0";
        let arcPath3 = "M" + (cx- cx / 2.5 + r/2) + "," + (cy - cy / 4.5) + " a1, 0.2, 0, 0,1 " + (r / 2) + ",0";
        let arcPath4 = "M" + (cx- cx / 2.5 + r/2) + "," + (cy - cy / 4.5) + " a1, 0.2, 0, 0,0 " + (r / 2) + ",0";
      ...
      if(shape=="instructionSVG2") {
        d3.select("#" + shape).append("line").attr("x1", cx - cx / 6).attr("x2", cx - cx / 2).attr("y1",cy - cy / 2).attr("y2",cy - cy+8 ).attr("stroke", "red").style("stroke-width", 1)
        ...
        d3.select("#" + shape).append("line").attr("x1", cx+cx/4 ).attr("y1", cy - cy / 4.5).attr("x2", cx+cx/1.2 ).attr("y2", cy - cy / 4.5).attr("stroke", "red").style("stroke-width", 1)
        ...
        d3.select("#" + shape).append("text").attr("x",cx+cx/1.2).attr("y", cy - cy / 5.5).text("Feature 2").attr("fill", "red").style("font-size", sizeOfPixel+"px")
        d3.select("#" + shape).append("text").attr("x",(2*cx) ).attr("y", cy + cy / 2.5).text("Feature 3").attr("fill", "red").style("font-size", sizeOfPixel+"px")
        d3.select("#" + shape).append("text").attr("x",cx-(cx/1.2)).attr("y",cy+cy+5).text("Feature 4").attr("fill", "red").style("font-size", sizeOfPixel+"px")
      }
  ...
  drawMan(shape:String,heightMan:number,widthMan:number,featureArray:any)
  {
    ...
    if(shape=="instructionSVG2")
    {
      ...
      d3.select("#" + shape).append("text").attr("x",faceCenterX - (9*torsoOffset)).attr("y",12.5*faceCenterY).text("Feature 4").attr("fill", "red").style("font-size", sizeofpixel+"px")
      ...
      d3.select("#" + shape).append("line").attr("x1", faceCenterX - torsoOffset + 3*bodyWidth).attr("x2",faceCenterX - torsoOffset + 3*bodyWidth).attr("y1",faceCenterY + 2*radiusFace).attr("y2",faceCenterY + 5*radiusFace).attr("stroke", "red").style("stroke-width", 1)
      d3.select("#" + shape).append("text").attr("x",faceCenterX - torsoOffset + 2*bodyWidth).attr("y",faceCenterY+(6.2*radiusFace)).text("Feature 3").attr("fill", "red").style("font-size", sizeofpixel+"px")

    }

  }

  drawManScrambled(shape:String,heightMan:number,widthMan:number,featureArray:any)
  {
    if(shape!="instructionSVG2"){  d3.select("#"+shape).html("")}
    ...
    let width=widthMan;
    let faceCenterX=width/2;
    let faceCenterY=(height/2)+(height/20);
    let radiusFace=height/20;
    let torsoOffset=width/15;
    let bodyHeight=(7.5/20)*height
    let bodyWidth=width/7.5;
    let armOffset=width/2;
    let sizeofpixel=7;
    ...
    if(featureArray[3]) {
      ...
      let armWidth = (faceCenterX - torsoOffset) - (faceCenterX - armOffset);
      let armheight = height / 20;
      let yArm = faceCenterY + radiusFace+bodyHeight-armheight;
      let x1 = faceCenterX - torsoOffset + bodyWidth;
      d3.select("#" + shape).append("rect").attr("x", xArm).attr("y", yArm).attr("width", armWidth).attr("height", armheight).attr("stroke", "black").style("fill", "white");
      //d3.select("#" + shape).append("rect").attr("x", x1).attr("y", yArm).attr("width", armWidth).attr("height", armheight).attr("stroke", "black").style("fill", "white");
    }
    if(featureArray[2]) {
      ...
      let armWidth = (faceCenterX - torsoOffset) - (faceCenterX - armOffset);
      let armheight = height / 20;
      let yArm = faceCenterY + radiusFace+bodyHeight/2-armheight;
      let x1 = faceCenterX - torsoOffset + bodyWidth;
      d3.select("#" + shape).append("rect").attr("x", x1).attr("y", yArm).attr("width", armWidth).attr("height", armheight).attr("stroke", "black").style("fill", "white");
    ...
    if(shape=="instructionSVG2")
    {
      ...
      d3.select("#" + shape).append("line").attr("x1", faceCenterX+radiusFace).attr("x2", faceCenterX+(bodyWidth/2)+2*torsoOffset+3*radiusFace).attr("y1",faceCenterY ).attr("y2",faceCenterY).attr("stroke", "red").style("stroke-width", 1)
      //d3.select("#" + shape).append("line").attr("x1", faceCenterX - torsoOffset).attr("x2", faceCenterX - (4*torsoOffset)).attr("y1",12*faceCenterY).attr("y2",12*faceCenterY).attr("stroke", "red").style("stroke-width", 1)
      d3.select("#" + shape).append("line").attr("x1", faceCenterX - torsoOffset ).attr("x2",faceCenterX - (6*radiusFace)).attr("y1",faceCenterY - (2*radiusFace)).attr("y2",faceCenterY - (2*radiusFace)).attr("stroke", "red").style("stroke-width", 1)
      d3.select("#" + shape).append("line").attr("x1", faceCenterX - armOffset).attr("x2",faceCenterX - armOffset-(armOffset/4)).attr("y1",faceCenterY + radiusFace+bodyHeight-height/40).attr("y2",faceCenterY + radiusFace+bodyHeight-height/40).attr("stroke", "red").style("stroke-width", 1)
      ...
      d3.select("#" + shape).append("text").attr("x",faceCenterX + 7*radiusFace).attr("y",faceCenterY+2).text("Feature 2").attr("fill", "red").style("font-size", sizeofpixel+"px")
      ...
      d3.select("#" + shape).append("text").attr("x",faceCenterX - 12.2*radiusFace).attr("y",faceCenterY-(1.5*radiusFace)).text("Feature 1").attr("fill", "red").style("font-size", sizeofpixel+"px")
      d3.select("#" + shape).append("text").attr("x",faceCenterX - armOffset-(armOffset/1.5)-2.2*radiusFace).attr("y",faceCenterY + 1.5*radiusFace+bodyHeight-height/40).text("Feature 4").attr("fill", "red").style("font-size", sizeofpixel+"px")


      d3.select("#" + shape).append("line").attr("x1", faceCenterX - torsoOffset + 3*bodyWidth).attr("x2",faceCenterX - torsoOffset + 3*bodyWidth).attr("y1",faceCenterY + radiusFace+bodyHeight/2).attr("y2",faceCenterY + radiusFace+bodyHeight/2+2*radiusFace).attr("stroke", "red").style("stroke-width", 1)
      d3.select("#" + shape).append("text").attr("x",faceCenterX - torsoOffset + 2*bodyWidth).attr("y",faceCenterY + radiusFace+bodyHeight/2+3.2*radiusFace).text("Feature 3").attr("fill", "red").style("font-size", sizeofpixel+"px")
    ...
    // if(featureArray[0]) {
    //   //Face of the totempole
    //   let cx = faceCenterX-(2.4*radiusFace)
    //   let cy = faceCenterY+(2*radiusFace)
    //   let r = radiusFace
    //   d3.select("#" + shape).append("circle").attr("cx", cx).attr("cy", cy).attr("r", r).attr("stroke", "black").style("fill", "white");
    // }
    // //Torso drawing
    ...
    //
    //
    //   d3.select("#" + shape).append("line").attr("x1", faceCenterX+(bodyWidth/2)+2*torsoOffset).attr("x2", faceCenterX+(bodyWidth/2)+2*torsoOffset+3*radiusFace).attr("y1",faceCenterY + (2*radiusFace)).attr("y2",faceCenterY + (2*radiusFace)).attr("stroke", "red").style("stroke-width", 1)
    //   d3.select("#" + shape).append("line").attr("x1", faceCenterX - torsoOffset).attr("x2", faceCenterX - (4*torsoOffset)).attr("y1",12*faceCenterY).attr("y2",12*faceCenterY).attr("stroke", "red").style("stroke-width", 1)
    //   d3.select("#" + shape).append("line").attr("x1", faceCenterX - (3*radiusFace) ).attr("x2",faceCenterX - (6*radiusFace)).attr("y1",faceCenterY + (2*radiusFace)).attr("y2",faceCenterY + (2*radiusFace)).attr("stroke", "red").style("stroke-width", 1)
    //   d3.select("#" + shape).append("line").attr("x1", faceCenterX - armOffset).attr("x2",faceCenterX - armOffset-(armOffset/4)).attr("y1",faceCenterY + radiusFace+bodyHeight-height/40).attr("y2",faceCenterY + radiusFace+bodyHeight-height/40).attr("stroke", "red").style("stroke-width", 1)
    //   //d3.select("#" + shape).append("line").attr("x1", faceCenterX - armOffset+(6*bodyWidth)).attr("x2",faceCenterX - armOffset+(6*bodyWidth)).attr("y1",faceCenterY + radiusFace).attr("y2",faceCenterY + radiusFace+(bodyHeight/2)).attr("stroke", "red").style("stroke-width", 1)
    //
    //   d3.select("#" + shape).append("text").attr("x",faceCenterX + 7*radiusFace).attr("y",faceCenterY+(2.5*radiusFace)).text("Feature 2").attr("fill", "red").style("font-size", sizeofpixel+"px")
    //   d3.select("#" + shape).append("text").attr("x",faceCenterX - (9*torsoOffset)).attr("y",12.5*faceCenterY).text("Feature 3").attr("fill", "red").style("font-size", sizeofpixel+"px")
    //   d3.select("#" + shape).append("text").attr("x",faceCenterX - 12.2*radiusFace).attr("y",faceCenterY+(2.5*radiusFace)).text("Feature 1").attr("fill", "red").style("font-size", sizeofpixel+"px")
    //   d3.select("#" + shape).append("text").attr("x",faceCenterX - armOffset-(armOffset/1.5)-2.2*radiusFace).attr("y",faceCenterY + 1.5*radiusFace+bodyHeight-height/40).text("Feature 3").attr("fill", "red").style("font-size", sizeofpixel+"px")
    //   //d3.select("#" + shape).append("text").attr("x",faceCenterX - armOffset+(6*bodyWidth)-sizeofpixel).attr("y",faceCenterY + radiusFace+(bodyHeight/2)+10).text("Feature 2").attr("fill", "red").style("font-size", sizeofpixel+"px")
    //
    //   // d3.select("#" + shape).append("line").attr("x1", faceCenterX).attr("x2", faceCenterX+faceCenterX/4).attr("y1",faceCenterY).attr("y2",faceCenterY).attr("stroke", "red").style("stroke-width", 1)
    //   // d3.select("#" + shape).append("line").attr("x1", faceCenterX).attr("x2", faceCenterX+faceCenterX/2).attr("y1",12*faceCenterY).attr("y2",12*faceCenterY).attr("stroke", "red").style("stroke-width", 1)
  ...
  drawPetals(shape:string,height:number,width:number,featureArray:any)
  {
    ...
    if(shape=="instructionSVG2") {
      ...
      let wedges4=wedges.fourthWedge;
       ...
       d3.select("#" +shape).append("text").attr("x",wedges3.fourth.x/4).attr("y", wedges3.fourth.y-(1.2*sizeOfPixel) ).text("Feature 1").attr("fill", "red").style("font-size", sizeOfPixel+"px")
       d3.select("#"+shape).append("line").attr("x1",wedges1.fourth.x).attr("x2",wedges1.fourth.x).attr("y1",wedges1.fourth.y-2*sizeOfPixel).attr("y2",wedges1.fourth.y+(1*sizeOfPixel)).attr("stroke", "red").style("stroke-width", 1)
       d3.select("#" + shape).append("text").attr("x",0.9*wedges1.fourth.x).attr("y", wedges1.fourth.y+(1.8*sizeOfPixel) ).text("Feature 3").attr("fill", "red").style("font-size", sizeOfPixel+"px")
       d3.select("#"+shape).append("line").attr("x1",wedges2.second.x).attr("x2",wedges2.second.x).attr("y1",wedges2.second.y-(2*sizeOfPixel)).attr("y2",wedges2.second.y+(sizeOfPixel)).attr("stroke", "red").style("stroke-width", 1)
       d3.select("#" + shape).append("text").attr("x",(3/4)*wedges2.second.x-((3/16)*wedges2.second.x)).attr("y",wedges2.second.y+(1.8*sizeOfPixel)).text("Feature 4").attr("fill", "red").style("font-size", sizeOfPixel+"px")
       d3.select("#"+shape).append("line").attr("x1",wedges4.second.x).attr("x2",wedges4.second.x).attr("y1",wedges3.fourth.y-(sizeOfPixel)).attr("y2",wedges3.fourth.y+(2*sizeOfPixel)).attr("stroke", "red").style("stroke-width", 1)
       d3.select("#" +shape).append("text").attr("x",wedges4.second.x/1.2).attr("y", wedges4.second.y-(0.8*sizeOfPixel) ).text("Feature 2").attr("fill", "red").style("font-size", sizeOfPixel+"px")
  ...
  drawWedges(cx:number,cy:number,r:number) : any
  {
    ...
    let thetaArray = [0.174533+0.174533,1.74533+0.174533, 3.31613+0.174533 ,4.88692+0.174533];
    ...
    let fourthWedge= this.getCoordinates(cx,cy,r,thetaArray[3])


    return {firstWedge:firstWedge, secondWedge:secondWedge, thirdWedge: thirdWedge,fourthWedge:fourthWedge}
  }
